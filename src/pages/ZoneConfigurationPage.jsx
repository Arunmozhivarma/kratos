...
export default function ZoneConfigurationPage() {
  ...
  const [currentMousePosition, setCurrentMousePosition] = useState(null); // Track current mouse position for drawing
  ...
  const drawZones = () => {
    ...
    if (isDrawing && startPoint && currentMousePosition) {
      ...
      ctx.lineWidth = 2;
      ctx.setLineDash([5, 5]);
      ctx.strokeRect(startPoint.x, startPoint.y, currentMousePosition.x - startPoint.x, currentMousePosition.y - startPoint.y);
      ctx.setLineDash([]);
    }
  };
  ...
  useEffect(() => {
    drawZones();
  }, [zones, isDrawing, currentMousePosition, currentConfigBox]);
                  ...
                  <div className="flex gap-2">
                    {[1, 2].map(boxNum => (
                      <button
                        ...
                        className={`px-4 py-2 rounded-lg font-medium transition ${currentConfigBox === boxNum
                    ...
                    return (
                      <div key={configBoxNum} className="border rounded-xl p-4 dark:border-gray-600">
                        <h4 className="font-medium text-primary mb-3 flex items-center">
                          <span className={`inline-block w-3 h-3 rounded-full mr-2 ${configBoxNum === 1 ? 'bg-emerald-500' : 'bg-orange-500'
                            }`} />
                        ...
                        {configBoxZones.length === 0 ? (
                          ...
                          <div className="space-y-2">
                            {configBoxZones.map(([zoneKey]) => {
                              ...
                              return (
                                <div key={zoneKey} className={`rounded-lg border px-3 py-2 ${configBoxNum === 1
                                  ? 'border-emerald-200 bg-emerald-50'
                                  : 'border-orange-200 bg-orange-50'
                                  }`}>
                                  <span className={`font-medium ${configBoxNum === 1
